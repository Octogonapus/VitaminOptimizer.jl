...
function buildAndOptimizeModel!(model::Model, limb::Limb, motors, gearRatios, filename::String)
	...
	register(model, :motorSlotτ, 1, motorSlotτ, autodiff=true)
	register(model, :motorSlotω, 1, motorSlotω, autodiff=true)
	register(model, :motorSlotPrice, 1, motorSlotPrice, autodiff=true)
	register(model, :motorSlotMass, 1, motorSlotMass, autodiff=true)
	register(model, :motorSlotGearRatio, 1, motorSlotGearRatio, autodiff=true)
	...
	# Equation 2
	@NLexpression(model, τ1Required, limb.tipForce * (link1 / 1000 + link2 / 1000 + link3 / 1000) +
								   gravity * (motorSlotMass(2) * link1 / 1000 +
								   motorSlotMass(3) * (link1 / 1000) + motorSlotMass(3) * (link2 / 1000)))
	@NLconstraint(model, eq3, motorSlotτ(1) >= τ1Required)

	# Equation 3
	@NLexpression(model, τ2Required, limb.tipForce * (link2 / 1000 + link3 / 1000) +
								   gravity * motorSlotMass(3) * (link2 / 1000))
	@NLconstraint(model, eq4, motorSlotτ(2) >= τ2Required)

	# Equation 4
	@NLexpression(model, τ3Required, limb.tipForce * (link3 / 1000))
	@NLconstraint(model, eq5, motorSlotτ(3) >= τ3Required)

	# Equation 5
	@NLconstraint(model, eq6, motorSlotω(1) * (link1 / 1000 + link2 / 1000 + link3 / 1000) >= limb.tipVelocity)

	# Equation 6
	@NLconstraint(model, eq7, motorSlotω(2) * (link2 / 1000 + link3 / 1000) >= limb.tipVelocity)

	# Equation 7
	@NLconstraint(model, eq8, motorSlotω(3) * (link3 / 1000) >= limb.tipVelocity)

	@NLexpression(model, θ3, π - atan(limb.targetZ / limb.targetX))
	...
	@NLexpression(model, θ4, asin(((link2/1000)*sin(β))/h))
	@NLexpression(model, α, (π - β - θ4) + (π - θ1 - θ3))

	objectiveFunction = sum(i -> motorSlotPrice(i), 1:length(slots))
	@NLobjective(model, Min, sum(motorSlotPrice(i) for i in 1:length(slots)))
...
solution = loadAndOptimzeAtParetoFrontier!(
    #makeGLPKModel(),
    #makeGurobiModel(1),
	#Model(with_optimizer(Ipopt.Optimizer)),
	Model(with_optimizer(NLopt.NLoptSolver(algorithm=:LD_MMA))),
